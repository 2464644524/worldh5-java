...
2. **B 方案：无任务计数保底（用户最终纠正版）。**
   - 单号自动任务确认运行后，固定 **每 3 分钟检查一次** 是否存在游戏原生 AutoGamer 可处理的 `CAN_ACCEPT`（可接）或 `CAN_SUBMIT`（可交）任务。
   - 当次检查没有自动可接/可交任务：无任务计数 `+1`。
   - 当次检查仍有自动可接或可交任务：无任务计数立即清零，继续挂机。
   - 无任务计数累计满 **20 次**才执行收尾并切号；20 次 × 3 分钟 = 约 60 分钟保底。
   - 城市功能/介绍/公告、支线、日常、随机、押镖、直交、无限提交、嵌套任务等原生 AutoGamer 不自动处理的任务，不计入自动任务。
   - 快照读取失败不计数，60 秒后重试。

3. **最大风险保护：** B 方案通常在连续约 60 分钟无自动任务后切号；网络/页面读取异常不计入 20 次，不会因为瞬时异常误切。
...
## 16. 天宇加载页首次点击勘误与修复（2026-09-17 23:27:30 CST）

用户确认 22:49 日志中的真实顺序：

1. 打开天宇直链后处于“页面加载中”；
2. 第一次 Canvas 点击 `(0.436, 0.870)` 发生在**加载页**，按钮含义是“进入游戏”，点击后才到选择角色界面；
3. 第二次 Canvas 点击 `(0.557, 0.929)` 才是在选择角色界面点击“进入游戏”，随后游戏核心对象就绪。

因此第 15 节中“第一次点击角色卡片”的理解是错误的，已按真实流程修复：

- 登录轮询改为扫描所有页面/所有游戏 Frame，不再只依赖单个最佳 Frame；
- `loading` 阶段且检测到可见 Canvas 后，天宇直链等待约 2.5 秒自动点击 `(0.436,0.870)` 的加载页“进入游戏”按钮；若仍未进入选角页，每 1.5 秒补点一次；
- 进入 `role` 阶段后，优先调用游戏对象 `selectRoleScene.onEnterGame(null)`；若约 0.6 秒后仍在选角界面，再点击 `(0.557,0.929)`；
- 若仍停留在选角页，会循环重试；
...
## 18. 自动任务过滤与无任务自动切号（2026-09-18 21:10 CST）

用户日志确认：`20:10` 后任务快照长期显示 `可接=14 可交=0`，但这 14 个是回城后的城市功能、介绍、支线/日常任务（如城市管理、加工、国家发展证、副本介绍等），不能作为自动主线仍可继续的依据。同时，上一提交 `df2caa3` 只实现了只读快照，没有真正关闭/切换账号。

本次补齐：

- `scripts/mission_snapshot.js` 在保留原始 `canAccept/canSubmit` 诊断字段的同时，新增自动任务专用结果：
  - `autoCanAcceptCount / autoCanSubmitCount`
  - `autoCanAccept / autoCanSubmit`
  - `ignoredNonAutoCount / ignoredNonAuto`
- 自动候选规则对齐游戏原生 `AutoGamer.isMissionCanDo`：
  - 已接取在身（`xself.getMissionById(id)`）直接算可自动处理；
  - `CAN_SUBMIT` 可交任务始终算仍有任务；
  - 未接 `CAN_ACCEPT` 任务排除已完成、一键任务、城市公告/介绍、国家分配、直交、无限提交、嵌套任务、押镖、随机任务。
- `SessionManager` 在托管自动任务确认运行后，固定每 3 分钟读取一次自动任务快照：
  - 有自动可接或可交：计数清零；
  - 两者都没有：`noMissionStreak + 1`；
  - 连续 20 次无任务（20 × 3 分钟，约 60 分钟保底）：停止当前脚本、关闭当前账号并切换队列下一个账号；
  - 快照读取失败不计数，60 秒后重试。
- 导号后会记住本次导号槽位顺序；从 A/C/D 队列中的某个账号点托管，会从当前账号开始，再按队列切换到后续账号。手动打开时使用具备天宇直链或官服账号密码的槽位生成兜底队列。
- 已打开的下一个账号调用 `AccountSession.open()` 会直接复用并前置，不重复启动 Edge；队列全部处理完后停止托管并输出“队列账号已全部处理完成”。
...
生效条件：必须完全关闭旧控制台和由它打开的所有 Edge 游戏窗口，再重新运行 `run.cmd`。

---

## 19. 批量“全托”入口（2026-09-18 23:18 CST）

需求澄清：用户要的是导号选中的 5 个账号都纳入托管，并在这 5 个账号之间自动切号，而不是只能先选中某一个槽位手动点单号“托管”。

实现：

- `WorldDesktop` 的“批量”区新增 **全托** 按钮；点击后有确认弹窗，随后调用 `SessionManager.startAutoPilotAll()`。
- `startAutoPilotAll()` 优先使用本次导号保存的 `lastLaunchedSlots`，从第一个槽位开始；没有导号队列时，才扫描本地已具备天宇直链或官服账号密码的槽位。
- 单号“托管”仍保留：从当前选中账号开始建立同一条队列。
- UI 运行中显示队列进度，如 `停管 1/5·01`，切换后变为 `停管 2/5·02`。
